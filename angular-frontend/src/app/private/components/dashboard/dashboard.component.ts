...
export class DashboardComponent implements OnInit{
  ...
  ngOnInit() {
    ...
    this.items$.pipe(
      tap((items) => {
        this.backlogItems = items.filter(item => item.status === 'A FAZER');
        this.todoItems = items.filter(item => item.status === 'EM PROGRESSO');
        this.doneItems = items.filter(item => item.status === 'CONCLUIDO');
      })
    ).subscribe();
  }
